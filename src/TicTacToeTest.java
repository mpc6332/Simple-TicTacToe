...
public class TicTacToeTest {
    ...
    * are recorded in an array list to check be for the game's winning conditions
    * later on.
    */
...

   {
      ...
      // Welcomes the player into the game along with displaying the instructions and
      // current score
      System.out.println("Welcome to tic-tac-toe!");
      System.out
            .println("Instructions: Here are the number placements that correspond to the position on the game board:");
      ...
      while (true) {
         ...
         // Allows the user to input anything when prompted to within parts of the
         // program.
         Scanner keyboard = new Scanner(System.in);

         /*
          * Prompts the user to pick out any number from 1 to 9 for placement. Number
          * placement goes like this in accordance to the board:
          * Top Row: [1], [2], [3]
          * Middle Row: [4], [5], [6]
          * Bottom Row: [7], [8], [9]
          * Then, the program repeatedly prompts the user until they input a number that
          * isn't already taken up by themselves or the cpu.
          */
         System.out.println("\nPlayer's turn! Please enter placement between 1 - 9");

         /*
          * After the program asks the user for input, the user puts in a number in that
          * specific range.
          * Then it is checked through the playerMarkCheck method to see if the number is
          * actually in the
          * range. Not to mention, it also checks if the user's attempted placement is
          * already on a spot
          * taken by the user themselves or the CPU. After all conditions are cleared, a
          * result is sent
         ...
         // Prints the updated game board with the gameBoard method, which shows the
         // player's latest successful input.
         printGameBoard(gameBoard);

         /*
          * For the player's end, the if loop checks to see if the player had won after
          * they had made their latest placement.
          * The winnerCondition method is called and its return value is placed within a
          * variable. After the returned value of
          * the winnerCondition method is printed out, it will check to see if the
          * initial return value "" was changed. If the
          * return method's value is returned as "", meaning that no outcome was decided,
          * then the if statement is bypassed and
          * the game resumes as normal. However, If the return value is not "", meaning
          * that an outcome was determined, then it
          * will ask the user if they want to continue the game. Before continuing a new
          * game, the program adds a 1 to either
         ...
         if (!result1.equals("")) {
            ...
            /*
             * After an outcome in the round was decided, it asks the user if they want to
             * play again through the
             * resetGameBoard method, which resets the board when a player wants to play a
             * new round. If replied
             * with N or n, the method will send back a string, "Gamestop", to break the
             * gameloop, which stops
             * the game and calls out a method that prints of the scores and determines who
             * won the most amount
             * of rounds. If replied with Y or y, it will send back a string,
             * "GameContinue", alongside with
         ...
         /*
          * Calls the getRandomNumber method, which will return a random number
          * calculated
         ...
         // Calls moveTurn to print out the CPU's mark after it successfully took up a
         // slot.
         System.out.println();
         moveTurn(gameBoard, cpuPlacement, "CPU");

         // Prints the updated game board with the gameBoard method, which shows the
         // CPU's latest successful input.
         printGameBoard(gameBoard);

         /*
          * For the CPU's end, the if loop checks to see if the CPU had won after it made
         ...
         if (!result2.equals("")) {
             ...
             * resetting the board that will start a new round with the player going first.
             * No need for extra
             * code to be in the "GameContinue" section as the program loops to the
             * beginning of the while
             * true loop, which segways into the player's turn anyways (We wanted the player
             * to go first
             * regardless of the outcome). This is possible as the breakstatement wasn't
             * activated yet.
             */
   ...
   // End of the main method. Below are the outside methods called from within the
   // main method
   ...
   public static void programEndResults(int playerScore, int cpuScore) {
      System.out.print("\nFinal player Score: " + playerScore + "\tFinal CPU Score: " + cpuScore + "\n");
      if (playerScore > cpuScore)
         System.out.println("\nThe player has won the most amount of rounds! You win!");
      else if (playerScore < cpuScore)
         System.out.println("\nThe CPU has won the most amount of rounds! You lose!");
      else if (playerScore == cpuScore)
         System.out.println("\nEach party has won an equal amount of rounds! It's a tie!");
    ...
    * After the program asks the user for input, the user puts in a number in that
    * specific range from
    * the main. Then it is passed on as a parameter to the playerMarkCheck method
    * to see if it is actually
    * within the range. If the number isn't within that range, the program will
    * loop itself until the user
    * does that. Afterwards, it which is then checked via while loop if the user's
    * placement
    * was on a spot previously taken up by either the player themselves or the cpu.
    * If those
    * spots were previously taken up, the user is asked to to make another input.
    * The input
    * is forced to go through a range check again, via a nested while loop, before
    * it can be checked to see if it is
    * attempting to place a mark on a previously taken up spot. The loop continues
    * until the user
    * takes a spot that isn't taken by either of the two parties. It then returns
    * the integer back
    * to the main for that number representing a slot on the board to be printed
    * out.
    * 
   ...
   public static int playerMarkCheck(int userPlacement) {
      ...
      while ((userPlacement < 1) || (userPlacement > 9)) {
         System.out.println("\nPlease only type in any number within the 1 - 9 range");
         userPlacement = keyboard.nextInt();
      }
   ...
   /**
    * This method places a player's mark on a gameBoard. It checks to see if the
    * method
    * being called is after a player's or CPU's turn is done through the parameter.
    * After
   ...
   /**
    * This method resets the game board depending if the player wants to continue
    * playing or not.
    * After being asked that question in the main method, it checks to see if the
    * user's input is
    * anything different than the following. If the input is different than expect,
    * the program
    * will keep asking to player to enter in an acceptable input until they do. If
    * they do want
    * to continue playing, It will clear all of the slots on the board along with
    * the lists
    * containing the user's and cpu's placements so the decision from the last
    * round isn't
    * accidentally printed out. From there, it will send back the GameContinue
    * string to
    * the main to start off a new game. If they don't want to continue, the
    * GameStop
    * string will be sent back to the main to let it know that the program has to
    * stop.
    * 
    * @param gameBoard The 2D array game board
    */
   public static String resetGameBoard(char[][] gameBoard) {
      ...
      while ((!userResponse.equals("N")) && (!userResponse.equals("Y")) && (!userResponse.equals("n"))
            && (!userResponse.equals("y"))) {
         System.out.println(userResponse);
         System.out.println("\nPlease input Y / y for yes or N / n for No");
   ...
   /**
    * Establishes the winning condition. Checks to see if certain inputs prompted
    * by either party, which
    * is stored in their own slot list, matches those of that of the winning
    * conditions, which is also
    * stored within its own list. Basically, it is comparing each parties'
    * placements to see if it contains
    * a specific number group that matches a winning condition that also contains
    * that group of numbers.
    * If a winning outcome is decided, it returns that result back to the main to
    * be printed out. If not,
    * the game sends back a blank string result that will cause the program to move
    * onto the next party's
    * turn. No parameters, but it does return a result back to the main that is not
